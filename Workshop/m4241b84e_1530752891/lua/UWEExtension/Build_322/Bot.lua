...
function Bot:Initialize(forceTeam, active, tablePosition)
    ...
    -- Sanity check to avoid freezing the server by adding too many virtual clients
    if #gServerBots == kMaxBots then
        Log("Warning: The maximum amount of %s bots has been reached. Stopping any additional bots from getting initialized!", kMaxBots )
        Log("Traceback: %s", debug.traceback())
        return false
    end
