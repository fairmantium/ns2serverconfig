...
Log("UWE-EXTENSION bots/Bot.lua loaded.")
...
-- TEMPORARY
local debugBotManager = Server.GetConfigSetting("debug_bot_manager")
...
function Bot:Initialize(forceTeam, active, tablePosition)
    ...
    -- TEMPORARY DEBUG
    if debugBotManager then
        HPrint("Bot:Initialize()")
        HPrint("%s", debug.traceback())
    end
